...
- 작업의 정확성, 데이터 보존과 실제 위험을 확인하는 데 필요한 검증만 수행한다. 이미 승인된 범위에 불필요한 gate, 반복 승인, 같은 조건의 재검증을 추가하지 않는다.
- 설명과 문서는 짧고 평이하게 쓴다. 독자가 이전 대화를 안다고 가정하지 않고, 결과를 이해하는 데 필요한 용어와 비교 기준을 처음 사용할 때 정의한다.
