...
7. 논문·연구 figure이면 [`guides/technical-figure-code.md`](guides/technical-figure-code.md)와 [`guides/scientific-figure-generation.md`](guides/scientific-figure-generation.md)
8. long-form Markdown authoring이면 [`guides/authoring-editor.md`](guides/authoring-editor.md)
9. 대상 프로젝트 자체의 `AGENTS.md`와 문서
...
MCP는 선택 사항입니다. Codex CLI나 MCP를 설치할 수 없는 VS Code Remote-SSH server에서는 이 repository를 clone한 뒤 작업 전에 `git pull --ff-only`하고, target project의 `AGENTS.md`가 clone의 absolute path와 필요한 Markdown read order를 직접 가리키게 합니다.
