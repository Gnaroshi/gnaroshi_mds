...
- Neutral main identity에서는 얼굴, 눈, 귀와 주요 silhouette를 가리지 않는다. App variant에서는 role foreground가 mascot의 아래 얼굴이나 몸을 가릴 수 있지만, 뒤에서 보이는 양쪽 귀와 눈이 같은 family임을 확인할 수 있어야 한다.
...
- App variant의 mascot은 완전한 bust나 별도 crest를 반복할 필요가 없다. Tile 뒤쪽 중앙에서 약 25–35%가 보이게 두고 role foreground와 자연스럽게 겹칠 수 있다.
- Role treatment는 tile 전면의 약 55–70%를 차지하는 한 개의 concrete workflow subject로 만든다. 여러 miniature control을 모은 dashboard가 아니라 실제 user action과 object가 함께 보이는 한 장면이어야 한다.
- Role foreground가 mascot보다 먼저 보여야 한다. 다만 남아 있는 귀·눈·teal/orange cue가 다른 character로 바뀌지 않게 한다.
...
- Current role family는 `overlap-workflow` composition을 사용한다. 같은 크기의 mascot crop을 tile 뒤쪽 중앙에 두고, 같은 깊이와 bounding box의 role subject가 아래에서 겹쳐 올라온다.
- Mascot을 별도 top crest처럼 role subject와 분리하지 않는다. 양쪽 귀와 눈 일부가 foreground 뒤에서 보이는 하나의 layered silhouette로 만든다.
- Role subject는 약 `38–44px` bounding box, 같은 2px outline, corner radius, perspective와 내부 spacing을 사용한다. 추상 badge 하나가 아니라 app의 대표 object에 대표 action이 일어나는 concrete scene이어야 한다.
- Tile border, background, mascot crop, overlap depth와 role-subject anchor는 family 전체에서 동일해야 한다. Key color와 subject만 바꾸어 frame geometry나 lighting이 달라지지 않게 한다.
...
- Icon은 mascot portrait나 rebus puzzle이 아니라 application launcher로 읽혀야 한다. 이름을 가린 32px 비교에서 foreground만 보고 대표 workflow를 말할 수 없다면 metaphor를 다시 설계한다.
...
| --- | --- | --- | --- |
| Gnaroshi Studio | 여러 작업 node가 연결된 중앙 authoring workbench와 한 개의 pen-nib/publish action | lavender `#B8A7F3` | mint `#8FD9C0` |
| PaperFlow | 실제 paper sheet가 guided conveyor를 따라 indexed library tray로 분류되는 장면 | mint `#8FD9C0` | sky `#82C7EE` |
| Arxiv Discovery | radar/scan sweep가 여러 incoming research paper를 찾아 밝히는 장면 | sky `#82C7EE` | peach `#F2B58D` |
| TR GPU Monitor | recognizable GPU card 위의 live telemetry와 연결된 remote-host signal | soft coral `#E9948E` | aqua/teal `#3FA6A0` |
| RunShelf | experiment vessel/run cue에서 나온 metric result가 indexed ledger shelf에 축적되는 장면 | butter yellow `#E9D27A` | teal `#3FA6A0` |
| ContentDeck | subtitle가 켜진 media frame 안에서 선택된 구간이 반복 학습되는 장면 | peach `#F2B58D` | lavender `#B8A7F3` |
