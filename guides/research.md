...
- 장시간 GPU 학습은 사용자가 직접 시작할 수 있는 실행 명령을 먼저 제공한다. 사용자가 tmux pane별 명령을 요청하면 각 pane에 붙여 넣을 완결된 명령을 준다. 별도 요청 없이 학습을 시작하거나 계속 상태를 조회·대기하지 않는다.
...
- 만점 피드백은 `잘 이해하셨습니다.`로 작성하며 이모티콘을 붙이지 않는다. 감점 피드백은 `[실습제출][점수/만점]`으로 시작하고 문항 구분을 유지한다.
