...
- Excel 피드백은 평가 영역마다 `[영역명][획득점수/영역만점]`을 표시한다. 문제풀이와 실습제출로 구성된 과제는 두 영역을 모두 적고, 영역이 만점이어도 점수 표시나 해당 영역을 생략하지 않는다.
- 영역 만점 피드백은 점수 표시 아래 `잘 이해하셨습니다.`로 작성하며 이모티콘을 붙이지 않는다. 감점 영역은 문항 구분을 유지하며 해당 오답이나 제출 누락을 설명한다.
