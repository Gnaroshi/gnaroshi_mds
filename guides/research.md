...
- 전체 총점이 만점인 학생의 Excel 피드백은 `잘 이해하셨습니다.` 한 문장만 적는다. 이 경우 영역별 점수나 피드백은 덧붙이지 않으며, 이모티콘을 붙이지 않는다.
- 감점 학생에게는 모든 평가 영역의 `[영역명][획득점수/영역만점]`과 피드백을 함께 표시한다. 문제풀이와 실습제출로 구성된 과제는 두 영역을 모두 적고, 그중 만점 영역도 생략하지 않는다. 만점 영역은 점수 표시 아래 `잘 이해하셨습니다.`를 적고, 감점 영역은 문항 구분을 유지하며 해당 오답이나 제출 누락을 설명한다.
