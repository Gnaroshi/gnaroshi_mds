...
이 guide는 논문, 연구 note와 발표 자료의 기술 도식을 code로 제작해 빠르게 검증하고 반복 export하는 선택적 baseline track이다. 여기의 evidence와 semantic audit는 Figma, slide tool 또는 raster editor에서 만드는 polished constructed schematic에도 동일하게 적용한다.
...
Code-based figure는 시각적으로 평범할 수 있다. 정확성과 편집성이 중요한 작업에서 즉시 사용할 수 있는 결과를 제공한다. 더 아름다운 technical figure가 필요하면 이 baseline의 evidence와 topology를 잠근 뒤 constructed polish pass로 이동한다. Image-generation model로 technical graph를 다시 해석하지 않는다.
...
- 사용자가 final raster 또는 PNG를 요청했더라도 architecture, pipeline, operator, state와 exact label이 핵심이면 constructed technical figure를 만든 뒤 raster로 export한다.
- 사용자가 image-generation model로 cover, teaser 또는 non-technical concept illustration을 요청했을 때만 `scientific-figure-generation.md`를 primary track으로 사용한다.
- 사용자가 두 방식을 함께 요청했을 때만 code와 generated 결과를 나란히 만든다.
- 사용자가 “못생겨도 바로 쓰는 것”과 “아름다운 technical final”을 함께 요청하면 `Code baseline`과 동일 evidence map을 사용하는 `Polished constructed schematic`을 나란히 만든다. Image-generated pipeline을 두 번째 결과로 사용하지 않는다.
- 두 guide를 모두 읽는 것은 경계를 이해하기 위한 것이며, 두 결과를 자동으로 만들라는 뜻이 아니다.
- Raster output과 image-generation method를 같은 뜻으로 취급하지 않는다.
...
Concept scene, cover 또는 teaser illustration이 목적이면 이 track으로 자동 전환하지 않는다. 시각적으로 polished된 technical schematic은 이 track의 semantic contract를 유지한 채 constructed layout tool에서 완성한다.
...
- Figma scripting 또는 manual Figma layout
- raster editor의 manual layout and typesetting

Mermaid는 관계를 빠르게 검토하는 planning artifact로만 사용한다. Mermaid 기본 render를 publication figure로 제출하지 않는다.

Production method와 final format을 분리한다.

- Code, Figma 또는 manual layout으로 만든 schematic도 PNG로 export할 수 있다.
- PNG라는 이유로 image-generation model을 사용하지 않는다.
- Target venue 또는 사용자가 format을 지정하면 이를 따른다.
- Format이 지정되지 않으면 editable source, high-resolution PNG와 paper-size preview를 제공한다.
- Line, arrow와 text가 중심인 source는 가능한 한 editable layer를 유지한다.
...
Image-generation model은 technical topology를 만드는 도구가 아니다. Cover, teaser와 non-technical concept illustration의 범위는 `scientific-figure-generation.md`를 따른다.

## Paper-first size and resolution

Target venue specification이 항상 우선한다.
...
16:9는 paper figure의 기본값이 아니다. Slide, video 또는 website hero에만 사용하고, paper figure는 intended column width와 content에 맞춰 aspect ratio를 정한다.

Pixel dimension은 physical size와 dpi에서 계산한다. Low-resolution render를 나중에 확대해 publication quality라고 보고하지 않는다.

## Polished constructed schematic

시각적으로 평범한 baseline과 아름다운 technical final을 함께 원하면 다음 순서를 사용한다.

1. Code baseline과 evidence map을 semantic lock으로 만든다.
2. 모든 arrow, operator, time index, label과 panel scope를 승인한다.
3. Figma, slide tool 또는 raster editor에서 typography, spacing, hierarchy와 grouping을 다시 설계한다.
4. Topology와 wording을 바꾸지 않는다.
5. Final physical size, grayscale와 resolution을 다시 검증한다.

Polish pass는 generated apparatus, crystal, ribbon, portal 또는 blank label을 추가하는 단계가 아니다.

두 결과를 함께 요청한 경우 같은 paper placement에서 side-by-side로 검토하고, semantic topology가 동일한지 diff checklist를 제공한다.
...
- working size
- expected 약 89 mm single-column 또는 182 mm double-column placement, 혹은 target venue size
- 160 px thumbnail
- grayscale
...
- Image-generation model이 technical topology, connector 또는 module label을 결정했다.
- Fake benchmark image, fake result, pseudo-code 또는 pseudo-text를 사용한다.
- Paper placement를 확인하지 않고 16:9 website/slide hero를 기본 composition으로 사용한다.
...
- Final pixel dimensions가 target physical size와 required dpi를 충족한다.
...
> Figure 작업 전에 `<gnaroshi_mds-clone>`의 current branch와 dirty state를 확인하고 `main`을 `git pull --ff-only`로 최신화하라. `<gnaroshi_mds-clone>/AGENTS.md`, `guides/research.md`, `guides/technical-figure-code.md`, `guides/scientific-figure-generation.md`를 처음부터 다시 읽어라. Target repository의 current working tree를 source of truth로 조사하고 evidence map을 drawing 전에 작성하라. PNG/raster는 export format이며 image-generation method가 아니다. Architecture, pipeline, state, operator와 exact label은 flat constructed schematic으로 만들고 image model에 맡기지 마라. 16:9를 paper 기본값으로 사용하지 말고 target venue 또는 약 89/182 mm placement에서 검증하라. 모든 arrow, operator, time index와 wording을 final render에서 다시 추출해 evidence map과 대조하라.

`<gnaroshi_mds-clone>`은 SSH server에 clone한 absolute path로 바꾼다. Guidance clone은 read-only reference로 취급한다. MCP나 Codex CLI 설치는 필요하지 않다.

## MCP invocation

MCP가 이미 제공되는 환경에서만 다음 지시를 사용한다.

> `gnaroshi://index`, `gnaroshi://guides/research`, `gnaroshi://guides/technical-figure-code`, `gnaroshi://guides/scientific-figure-generation`을 먼저 읽어라. 현재 요청이 figure role과 산출물 수의 최상위 권한이다. PNG/raster와 image generation을 혼동하지 마라. 이전 figure의 content, layout과 wording을 그대로 재사용하지 말고, current user wording과 target repository의 current working tree를 조사하라. Training/inference entry point, core module의 signature와 actual call site, schema, config, loss, state update, optimizer/freezing, tests와 필요한 runtime trace를 확인하라. 모든 visible module, arrow, operator, time index, shared/frozen/detached state와 label마다 source file, symbol, producer→consumer, verification state를 기록한 evidence map을 drawing 전에 작성하라. Mean/add/concat/gating/residual 같은 operator와 `t`→`t+1` dependency를 이름이나 모양으로 추정하지 마라. Introduction figure라면 한 문장 claim과 main contribution을 가장 크게 두고 full architecture dump를 만들지 마라. Technical pipeline은 image model이 아니라 flat constructed 2D로 만들고 required label을 actual typeset text로 넣어라. 16:9를 paper 기본값으로 사용하지 마라. 마지막에 모든 arrow, operator, time index와 wording을 figure에서 추출해 evidence map과 line-by-line으로 다시 대조하라. 근거가 없는 관계는 그리지 말고 code/document mismatch와 미검증 항목을 보고하라.

Repository에 직접 접근할 수 없는 Codex에는 위 지시와 함께 `Implementation source bundle`의 file을 전달한다.

MCP는 optional convenience다. 이 guide를 적용하기 위해 서버에 Codex CLI나 MCP를 설치할 필요는 없다.

## Canonicality

- Reusable figure policy는 이 repository의 두 figure guide에만 둔다.
- Target project에는 figure-specific spec, caption, terminology와 evidence map만 둔다.
- Target output folder에 새로운 reusable guideline version을 만들지 않는다.
- Snapshot ZIP은 provenance artifact일 수 있지만 canonical guidance가 아니다.
...
- [RT-1 project and paper](https://robotics-transformer1.github.io/): real robot-task evidence와 labeled model/action explanation의 역할을 분리하는 robotics reference다.
- [OpenVLA project and paper](https://openvla.github.io/): real robot evaluation media와 model overview를 구분해 제시하는 VLA reference다.
- [ACT paper](https://tonyzhaozh.github.io/aloha/aloha.pdf): multi-view observation, state, latent와 action chunk 관계를 readable flat architecture로 설명하는 reference다.
...
Reading both figure guides does not authorize automatic paired output. The request controls role, final format and output count, while technical topology remains constructed and auditable regardless of whether the final file is raster.
...
### 2026-07-20 — Separate construction from export

PNG and raster are output formats, not permission to generate technical topology with an image model. Technical figures remain constructed, editable and auditable regardless of final export format.

### 2026-07-20 — Add a polished constructed path

Visual quality is improved after the evidence map and topology are locked, using Figma, slide or raster-layout tools without changing arrows, operators, labels or time semantics.

### 2026-07-20 — Restore paper-first dimensions

Target venue dimensions override generic presets. When unknown, approximately 89/182 mm and 300/600 dpi are working references; 16:9 is reserved for explicitly wide slide or web placements.
