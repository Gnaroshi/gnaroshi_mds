...
실제 구현을 설명하는 figure는 대상 repository의 current code와 검증 가능한 runtime behavior를 기술 source of truth로 사용한다. Paper, README와 architecture document는 intent와 terminology를 제공하지만 현재 구현을 자동으로 증명하지 않는다.

Codex는 composition을 정하기 전에 implementation source bundle을 읽고 figure evidence map을 작성한다.
...
2. 현재 branch, commit SHA와 dirty-state 여부
3. architecture 또는 method document
4. training entry point
5. inference 또는 evaluation entry point
6. figure에 등장할 core module과 interface/type definition
7. default config, experiment config와 feature flag
8. input, observation, state, output와 action schema
9. loss, objective, optimizer와 parameter-freezing 설정
10. cache, memory, recurrent state 또는 update logic
11. representative unit, integration 또는 shape test
12. privacy-safe sample input/output, trace 또는 tensor-shape log가 실제 behavior 확인에 필요할 때 해당 artifact

Codex가 repository를 직접 읽을 수 있으면 file을 복사해 보내는 대신 위 category의 path와 figure scope를 알려준다. Secret, private dataset, credential, unpublished raw result와 식별 가능한 log는 전달하지 않는다.

### Evidence map

Drawing을 시작하기 전에 최소한 다음 표를 만든다.

| Figure element | Source file | Symbol or config key | Test or runtime evidence | Confidence |
| --- | --- | --- | --- | --- |
| 표시할 module, state, path 또는 output | repository-relative path | class, function, field, flag 또는 target | test name, trace, shape check 또는 `not observed` | high, medium, low |

Evidence map은 review artifact다. Final figure 안에 source path를 노출할 필요는 없지만 deliverable과 함께 제공한다.

### Grounding rules

- Import 관계만으로 runtime data flow를 추정하지 않는다. Call site, dispatch, config와 representative test를 함께 확인한다.
- Module 이름만으로 input, output, tensor shape, frequency, sharing, cache, recurrence 또는 frozen/trainable 상태를 추정하지 않는다.
- Training과 inference path를 별도로 확인한다.
- Default config와 paper experiment config가 다르면 둘을 구분한다.
- `requires_grad`, optimizer parameter group, detached tensor, evaluation mode와 checkpoint loading을 확인하기 전에는 frozen 상태로 표시하지 않는다.
- Reuse, shared, cached, sparse, asynchronous 또는 optional이라고 표시하려면 해당 behavior를 만드는 code 또는 config 근거가 있어야 한다.
- Static inspection만으로 dynamic behavior를 확정할 수 없으면 focused test, dry run, trace 또는 shape logging을 사용한다.
- Code와 test가 현재 behavior를 보여주고 document가 다른 내용을 말하면 mismatch를 보고한다. 어느 쪽도 조용히 맞는 것으로 간주하지 않는다.
- 근거가 불충분한 element나 connector는 확정해서 그리지 않는다. `unverified`로 분리하거나 사용자에게 한 개의 구체적인 확인 질문을 한다.
...
> `gnaroshi://index`, `gnaroshi://guides/research`, `gnaroshi://guides/technical-figure-code`, `gnaroshi://guides/scientific-figure-generation`을 먼저 읽어라. 대상 repository의 current implementation을 source of truth로 조사하라. Training과 inference entry point, core module, type/schema, config, loss, state update, tests와 필요한 runtime evidence를 확인한 뒤 주요 visual element마다 source file, symbol, verification state를 기록한 evidence map을 먼저 작성하라. 근거가 없는 technical relationship을 확정하지 말고 code/document mismatch와 미검증 항목을 보고하라. 그 다음 즉시 사용할 수 있는 code baseline과 visual-quality-first generated candidate를 나란히 제시하라.
...
Figure brief와 implementation evidence map을 node, relationship, group, sequence, comparison, state와 annotation으로 정리한다.
...
- implementation evidence map과 inspected commit
...
- implementation-grounded figure의 모든 주요 element와 connector에 evidence map entry가 있다.
...
### 2026-07-19 — Ground implementation figures in executable evidence

When a figure describes an implemented system, inspect the current code, configuration, tests, and relevant runtime behavior before drawing. Maintain an evidence map from each major visual element to its source file, symbol, and verification state.
